...
<html lang="en">
    ...
    <style>
        ...
        <body>
            <h1>Sorted Students Records</h1>
        <table>
            <tr><th>ID</th><th>Name</th><th>Grade</th></tr>
        
        <?php $conn=new mysqli("localhost","root","1239","students124");

        $students=$conn->query("SELECT * FROM students124")->fetch_all
        (MYSQLI_ASSOC);
        ...
        {$student['name']}</td><td> {$student['grade']}</td></tr>";
        }
        $conn->close();
        ?>
        </table>
